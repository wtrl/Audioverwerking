...
fraction = 0.05*std(H(:,1));
H_noise = H + fraction.*randn(size(H));
%remove zero rows
ix = any(H_noise,2);
H_small = H_noise(ix,:);
...
for j=1:nrOfBinauralSigs
    for i=1:nrOfLoudspeakers
        Left(:,j) = Left(:,j) + fftfilt(H_L(:, 1+(i-1)*Lg : Lg*i)*g( 1+(i-1)*Lg : Lg*i,j), speech);
        Right(:,j) = Right(:,j) + fftfilt(H_R(:, 1+(i-1)*Lg : Lg*i)*g( 1+(i-1)*Lg : Lg*i,j), speech);
    end
end
